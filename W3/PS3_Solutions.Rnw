...
The correlation is bound between -1 and 1. The correlation value here should be very close to 1 or -1 but not be 1 due to the randomly distributed error. As long as some random error has been introduced to a formerly linear relationship, even if that relationship is still clearly linear, there will be a reduction in the absolute value of the correlation. The result you can expect to get here is an absolute value of approximately 0.99, but it depends on the size of the random error that you introduced. If you introduce a random error that has a greater standard deviation, then your correlation will go down even further.

Interpretation: a correlation close to 1 or -1 indicates a nearly perfect linear relationship of two variables. If the relationship is positive: as x goes above its mean, y goes above its mean. If the relationship is negative: as x goes above its mean, y goes below its mean. The randomly distributed error does not change the generally strongly linear relationship between the two.
...
There is a negative linear relationship between Fertility and Education. For a 1-point increase in Fertility, we expect a 0.41-point decrease in Education, holding all other variables at the same value. The t-value is -4.758. This t-value implies a p-value of $2.43*10^{-5}$. This $p < 0.001$, meaning that the statistical relationship is significant at all common levels of statistical significance.

The other variables are interpreted accordingly. The levels of significance can be found in the tutorial notes.
