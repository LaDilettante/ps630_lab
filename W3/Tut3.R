### Pol Sci 630: Into to Empirical Approaches
### Tutorial 3: Comparisons and Inference
### Prepared by: Jan Vogler (jan.vogler@duke.edu)



### Briefly: sometimes R gives you output like this:
...
### Let us create two variables that are clearly dependent on each other.
...
### Let us look more closely at the formula of covariance, using the first example.

sum((x - mean(x))*(y-mean(y)))/(length(x)-1) # Note that x and y must have the same length, so it doesn't matter which one you use in the denominator (as length(x)).
cov(x,y)

### Those two formulas return the same value.

### If we apply the first formula to x3 and y3, it becomes obvious, why the outcome is zero.

sum((x3 - mean(x3))*(y3-mean(y3)))/(length(x)-1)
...
### As you've learned in the lecture, the problem with covariance is that it is not to scale. It doesn't really tell us that much about how much variables vary with each other because it doesn't account for their individual variation.
...
### Interestingly, R allows us to find the correlation between a numeric and a logical vector, although a logical vector is not numeric.
...
### Let us assume that 1 is a low occupational status and 8 is a high occupational status. Is there a relationship between the status of the father and the son?
...
### Typically, a p-value is related to a type-1 error rate (alpha) that we define in advance. Often we want alpha to be smaller than 0.05.

### Typically, our null hypothesis (H0) is that there is no relationship between two variables.

### The p-value is the probability that we get data with evidence that is such strong AGAINST H0 if H0 was true. Think about what this means.
...
### The summary of this linear regression will return a t-value and a p-value for the inercept all coefficients.

summary(lm1)

### The last column Pr(>|t|) is the p-value of the t-test. How do we interpret this finding?

### The probability to find this data if H0 is true (i.e. there is no relationship between body and brain size in Animals) is 0.1%. This is strong evidence against H0. Therefore we reject H0 under p < 0.001. Please pay attention to the difference between percent (0-100) and proportions (0-1).

### For a 1-point increase in Solar.R, we would expect a 0.13 increase in Ozon (in a multivariate model we would have to add: "holding all other covariates constant"). The associated t-value is 3.880, which imples a p-value of 0.0002. This p-value is p < 0.001. Therefore, the relationship is statistically significant at all common levels of statistical significance.

### Note that there are four important thresholds of statistical significane:

p = 0.001 # equals a type-1 error rate (alpha) of 0.001
p = 0.01 # equals a tye-1 error rate (alpha) of 0.01
p = 0.05 # equals a type-1 error rate (alpha) of 0.05
p = 0.1 # equals a type-1 error rate (alpha) of 0.1

### If an indicator has a p-value of p < 0.001, it is significant at all common levels of statistical significance.

### This indicates that there is a significan t linear relationship between the two variables.

### If you have a linear regression with multiple independent variables, then it looks the following:
