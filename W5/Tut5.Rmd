...
An essential aspect of all linear models are the marginal effects that predictor variables are estimated to have on the response variable.
...
An important condition for using OLS is that the predictor variables are not subject to multicollinearity. Let's check multicollinearity. We need another package for this:
...
What can we conclude from these statistics? What can we say about the hypothesis that there is a linear relationship between "l1polity" and "newtar"?
...
2. Reviewers that demand you to include them
...
```{r}
main.res = resid(main)
plot(main.res, main="Valus of the Error Term")
```
...
The distribution of the errors is approximately normal, so this condition of OLS is met.
